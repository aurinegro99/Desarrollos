...
if ($_SERVER["REQUEST_METHOD"]==="POST"){
   ...
   require_once '../../phpexcel/Classes/PHPExcel.php';
    ...
    $libro->setActiveSheetIndex(0)->setCellValue('F3','Informe Consumos Conciliados');
    ...
    $libro->setActiveSheetIndex(0)->setCellValue('R7', 'Jefe Turno Clte.');
    $libro->setActiveSheetIndex(0)->setCellValue('S7', 'Fecha Aprob. Clte.');
    $libro->setActiveSheetIndex(0)->setCellValue('T7', 'IP computador Clte.');
    
    $xlista=$dato->getDatosListado($_POST["idfaena"], $_POST["contrato"], $_POST["inicio"], $_POST["termino"], 2);
    $y=0;
    foreach($xlista as $datos){
            ...
            $xaprob=$dato->getRegistroAprobacion($datos["idconsumo"],1);
            foreach($xaprob as $xlineas){
                   if ($xlineas["usuario_aprobacion"]=='0')
                      $libro->setActiveSheetIndex(0)->setCellValue('O'.(8+$y), 'Act. Planif. (Aprob. Automatica)');
                   else    
                      $libro->setActiveSheetIndex(0)->setCellValue('O'.(8+$y), $xlineas["nombre_usuario"]);
                    $libro->setActiveSheetIndex(0)->setCellValue('P'.(8+$y), $xlineas["fecha_aprobacion"]);
                    $libro->setActiveSheetIndex(0)->setCellValue('Q'.(8+$y), $xlineas["ip_computador"]);
            }
            
                
            $libro->setActiveSheetIndex(0)->setCellValue('R'.(8+$y), $datos["nombre_usuario"]);
            $libro->setActiveSheetIndex(0)->setCellValue('S'.(8+$y), $datos["fecha_aprobacion"]);
            $libro->setActiveSheetIndex(0)->setCellValue('T'.(8+$y), $datos["ip_computador"]);
 ...
 header('Content-Disposition: attachment;filename="Consumos_Conciliados.xlsx"');
