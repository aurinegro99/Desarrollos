...
function PoPCalCreateCalendarInstance()
{
	...
	if (!PopCalInstanceCreated())
	{
		...
		*/
		...
		this.imgDir = "../img/" // directory for images ... e.g. var imgDir="/img/"
		...
		this.styleAnchor="text-decoration:none;color:black;cursor:hand;"
		this.styleLightBorder="border-style:solid;border-width:1px;border-color:#A5B2CE;"
...
function PopCalInitCalendar()
{
	...
	if (PopCal)
	{
		...
		{
			if ((objPopCalendar.language > 2) || (objPopCalendar.language < 0))
			{
			...
			if (objPopCalendar.language == 2)
				{
					objPopCalendar.monthName = new Array("Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","August","Setembro","Outubro","Novembro","Dezembro")
	
					if (objPopCalendar.startAt==0)
					{
						objPopCalendar.dayName = new Array ("Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sabado")
					}
					else
					{
						objPopCalendar.dayName = new Array ("Segunda","Terça","Quarta","Quinta","Sexta","Sabado","Domingo")
					}
			...
			sHTML1="<span id='popupSuperSpanMonth' style='border-style:solid;border-width:1;border-color:#FFFFCC;cursor:default;' onmouseover='this.style.borderColor=\"#FFFF00\";window.status=\""+objPopCalendar.selectMonthMessage+"\"' onmouseout='this.style.borderColor=\"#FFFFCC\";window.status=\"\"' onclick='objPopCalendar.keepMonth=!PopCalIsVisible(objPopCalendar.crossMonthObj);PopCalUpMonth()'></span>&nbsp;"
			
			sHTML1+="<span id='popupSuperSpanYear' style='border-style:solid;border-width:1;border-color:#FFFFCC;cursor:default;' onmouseover='this.style.borderColor=\"#FFFF00\";window.status=\""+objPopCalendar.selectYearMessage+"\"'	onmouseout='this.style.borderColor=\"#FFFFCC\";window.status=\"\"' onclick='objPopCalendar.keepYear=!PopCalIsVisible(objPopCalendar.crossYearObj);PopCalUpYear()' onMouseWheel='PopCalWheelYear()'></span>&nbsp;"
			...
			if ( objPopCalendar.crossobj.visibility == "hidden" ) 
			{
				...
				if (from!=null)
				{
					if ((from.toLowerCase() == "hoy") || (from.toLowerCase() == "now"))
					{
						...
						//objPopCalendar.yearFrom = objPopCalendar.yearNow //Restringe Año
						objPopCalendar.yearFrom = 1990
					}
				...
				if (!PopCalCenturyOn(objPopCalendar.dateFormat))
				{
					...
					if (PopCalDateUpTo() >  PopCalPadZero4(1990 + (objPopCalendar.centuryLimit-1)) + "1131")
					{
						objPopCalendar.dateUpTo = 31
						objPopCalendar.monthUpTo = 11
						objPopCalendar.yearUpTo = 1990 + (objPopCalendar.centuryLimit-1)						
					}
				}
...
function PopCalDrop()
{
	objPopCalendar.bShow = true
	objPopCalendar.movePopCal = false
	document.getElementById("popupSuperHighLight").style.borderColor = "#ADC3FF"
}
...
function PopCalConstructMonth() 
{
	...
	if (!objPopCalendar.monthConstructed) 
	{
		...
		for (var i=0; i<=11; i++) 
		{
			...
			sHTML += "<tr><td id='popupSuperMonth" + i + "' onmouseover='objPopCalendar.bShow=true;this.style.backgroundColor=\"#ADC3FF\"' onmouseout='objPopCalendar.bShow=false;this.style.backgroundColor=\"\"' style='cursor:default' onclick='objPopCalendar.bShow=false;objPopCalendar.monthConstructed=false;objPopCalendar.monthSelected=" + i + ";PopCalConstructCalendar();PopCalDownMonth();event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>"
		}

		document.getElementById("popupSuperMonth").innerHTML = "<table width=70 style='font-family:arial; font-size:11px; border-width:1; border-style:solid; border-color:#A5B2CE;' bgcolor='#FFFFDD' cellspacing=0 onmouseover='clearTimeout(objPopCalendar.timeoutID1)' onmouseout='clearTimeout(objPopCalendar.timeoutID1);event.cancelBubble=true'>" + sHTML + "</table>"
...
function PopCalSelectYear(nYear) 
{
	...
	//el mes cambia al mes Actual cuando se selecciona el año actual
...
function PopCalConstructYear() 
{
	...
	if (!objPopCalendar.yearConstructed)
	{
		...
		{
			//beginYear	= objPopCalendar.yearFrom - 2 //Año Actual menos 2 años
			beginYear=1990
		...
		{
			//beginYear = objPopCalendar.yearFrom - 2
			beginYear=1990
		...
		if (longList) //Hacia  arriba la flecha
		{
			sHTML += "<tr><td align='center' onmouseover='objPopCalendar.bShow=true;this.style.backgroundColor=\"#ADC3FF\"' onmouseout='objPopCalendar.bShow=false;clearInterval(objPopCalendar.intervalID1);this.style.backgroundColor=\"\"' style='cursor:default;border-bottom:1px #A5B2CE solid' onmousedown='clearInterval(objPopCalendar.intervalID1);objPopCalendar.intervalID1=setInterval(\"PopCalDecYear()\",15)' onmouseup='clearInterval(objPopCalendar.intervalID1)'><IMG id='popupSuperUpYear' SRC='"+objPopCalendar.imgDir+"arriba.gif' BORDER=0></td></tr>"
		}

		var j =	0
		for (var i=(beginYear); i<=(endYear); i++) //Inicio de lista 
		{
			var sName =	i
			...
			sHTML += "<tr><td id='popupSuperYear" + j + "' align='center' onmouseover='objPopCalendar.bShow=true;this.style.backgroundColor=\"#ADC3FF\"' onmouseout='objPopCalendar.bShow=false;this.style.backgroundColor=\"\"' style='cursor:default' onclick='objPopCalendar.bShow=false;PopCalSelectYear("+j+");event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>"
			j ++
		}

		if (longList) //Fechas para Abajo
		{
			sHTML += "<tr><td align='center' onmouseover='objPopCalendar.bShow=true;this.style.backgroundColor=\"#ADC3FF\"' onmouseout='objPopCalendar.bShow=false;clearInterval(objPopCalendar.intervalID2);this.style.backgroundColor=\"\"' style='cursor:default;border-top:1px #A5B2CE solid' onmousedown='clearInterval(objPopCalendar.intervalID2);objPopCalendar.intervalID2=setInterval(\"PopCalIncYear()\",15)' onmouseup='clearInterval(objPopCalendar.intervalID2)'><IMG id='popupSuperDownYear' SRC='"+objPopCalendar.imgDir+"abajo.gif' BORDER=0></td></tr>"
		}

		document.getElementById("popupSuperYear").innerHTML	= "<table width=44 style='font-family:arial; font-size:11px; border-width:1; border-style:solid; border-color:#A5B2CE;'	bgcolor='#FFFFDD' onmouseover='clearTimeout(objPopCalendar.timeoutID2)' onmouseout='clearTimeout(objPopCalendar.timeoutID2);' cellspacing=0>"	+ sHTML	+ "</table>"
...
function PopCalUpYear() //Posicion de la Lista
{
...
function PopCalConstructCalendar ()
{
	...
	for (var datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
	{
		...
		for (var k=0;k<objPopCalendar.HolidaysCounter;k++)
		{
			if ((parseInt(objPopCalendar.Holidays[k].d, 10)==datePointer)&&(parseInt(objPopCalendar.Holidays[k].m, 10)==(objPopCalendar.monthSelected+1)))
			{
				if ((parseInt(objPopCalendar.Holidays[k].y)==0, 10)||((parseInt(objPopCalendar.Holidays[k].y, 10)==objPopCalendar.yearSelected))&&(parseInt(objPopCalendar.Holidays[k].y, 10)!=0))
				{
					...
					{
						sStyle+="background-color:#ADC3FF;"
					}
		...
		if (objPopCalendar.yearSelected == objPopCalendar.yearFrom && objPopCalendar.monthSelected == objPopCalendar.monthFrom)
		{
			...
			{
				// Elimina las Lineas en las fechas corresponidente al mes actual
				notSelect = false
			}
		}
		...
		if (objPopCalendar.yearSelected == objPopCalendar.yearUpTo && objPopCalendar.monthSelected == objPopCalendar.monthUpTo)
		{
			...
			{
				// Seleccionar dias del mes antes del actual
				notSelect = false
			}
		}
	...
	document.getElementById("popupSuperSpanMonth").innerHTML = "&nbsp;" + objPopCalendar.monthName[objPopCalendar.monthSelected] + "&nbsp;<IMG id='popupSuperChangeMonth' SRC='"+objPopCalendar.imgDir+"down.gif' WIDTH='12' HEIGHT='10' BORDER=0>"
	document.getElementById("popupSuperSpanYear").innerHTML = "&nbsp;" + objPopCalendar.yearSelected + "&nbsp;<IMG id='popupSuperChangeYear' SRC='"+objPopCalendar.imgDir+"down.gif' WIDTH='12' HEIGHT='10' BORDER=0>"
...
//	PopCalAddHoliday(18,4,0,"Viernes Santo", "Good Friday") // Sólo en Año 2003
//	PopCalAddHoliday(19,4,0,"S" + c_aTilde + "bado Santo", "Saturday Saint") // Sólo en Año 2003
